...
export default function Sponsors({ embedded = false }) {
  ...
  return (
    <div
      className={`${
        embedded ? "py-16" : "min-h-screen"
      } bg-transparent text-[#F4EBD9] relative`}
    >
      {!embedded && (
        <>
          <SocialSideRail />
              ...
              <div className="flex flex-wrap justify-center gap-6 max-w-5xl mx-auto">
                {tier.sponsors.map((sponsor, sponsorIndex) => (
                  ...
                  >
                    ...
                    <div className="relative w-full h-36 sm:h-44 bg-[#020812]/85 p-5 rounded-2xl flex items-center justify-center mb-5 border border-[#C5A25F]/20 shadow-inner group-hover:border-[#C5A25F]/45 transition-colors">
                      <img
                        src={sponsor.image}
                        alt={sponsor.name}
                        loading="lazy"
                        className="max-h-full max-w-full object-contain drop-shadow-[0_4px_14px_rgba(0,0,0,0.72)] group-hover:scale-[1.03] transition-transform duration-300"
                      />
                    </div>

                    <h4 className="relative font-cinzel font-bold text-base sm:text-lg text-[#F4EBD9] tracking-wide group-hover:text-[#DDB66F] transition-colors">
                      {sponsor.name}
                    </h4>

                    <span className="relative text-[10px] sm:text-xs text-[#9FC4D7] font-mono mt-2 uppercase tracking-[0.16em] px-3 py-1 rounded-full bg-[#0EA5E9]/10 border border-[#0EA5E9]/20">
                      {sponsor.category}
                    </span>
                  </button>
                ))}
